...
CACLCULATE:
		...
		DIV BX;折扣之后的实际加个
		MOV CX,AX;实际价格存到CX中
		MOV AX,[SI+11];AX此时为进货价格
		MOV BX,128;128存到BX中，等待与进货价格相乘
		MUL BX
		MOV DX,0
		DIV CX;式子的第一项算出来了
		PUSH AX;为了使用寄存器，就先把AX的值进栈
		MOV AX,[SI+17];已售数量
		MOV BX,64
		MUL BX	;式子第二项的分子算出来了
		MOV BX,[SI+15];进货数量
		MOV DX,0
		DIV BX;此时AX=已售数量*64/进货数量，现在只差第一项相加了
		POP BX;	把第一项的值出栈放到BX中
		ADD AX,BX;第二项与第一项就全部加起来了
		MOV [SI+19],AX ;把推荐度放进货物对应位置处
		ADD SI,21	;移动到下一个商品处
		DEC DI	;控制计算数量，计数器自减
		JNZ CACLCULATE
